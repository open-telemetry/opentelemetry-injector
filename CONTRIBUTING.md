...
The DEB and RPM system packages that bundle the injector with the Java, Node.js, and .NET auto-instrumentation agents now live in [open-telemetry/opentelemetry-packaging](https://github.com/open-telemetry/opentelemetry-packaging).
...
If a changelog entry is not required, add the `"Skip Changelog"` label to disable this action.
If your pull request title starts with `chore`, `test`, or `ci`, the action is skipped automatically.
