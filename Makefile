...
so/$(BINARY_NAME_NO_ARCH): so
	zig build -Dcpu-arch=${ARCH} -Dallowed-env-var-prefixes='$(ALLOWED_ENV_VAR_PREFIXES)' --summary none
...
zig-build: check-zig-installed
	@mkdir -p so
	@(zig build -Dcpu-arch=${ARCH} -Dallowed-env-var-prefixes='$(ALLOWED_ENV_VAR_PREFIXES)' --summary none && echo $(shell date) build successful) || (echo $(shell date) build failed && exit 1)
...
zig-unit-tests: check-zig-installed
	@(zig build test -Dcpu-arch=${ARCH} -Dallowed-env-var-prefixes='$(ALLOWED_ENV_VAR_PREFIXES)' --summary none && echo $(shell date) tests successful) || (echo $(shell date) tests failed && exit 1)
