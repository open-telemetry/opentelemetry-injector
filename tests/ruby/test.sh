...
docker buildx build -q --platform linux/${arch} --build-arg BASE=$BASE -o type=image,name=otelinject-test-ruby,push=false .
OUTPUT=$(docker run --platform linux/${arch} --rm otelinject-test-ruby)
echo "========== OUTPUT =========="
echo "$OUTPUT"
echo "============================"
...
echo "Test presence of RUBYOPT"
echo "$OUTPUT" | grep "RUBYOPT" > /dev/null && echo "Test passes"  || exit 1
...
# Check we didn't inject env vars into processes outside of ruby.
OUTPUT_BASH=$(docker run --platform linux/${arch} --rm otelinject-test-ruby /usr/bin/env)
echo "======= BASH OUTPUT ========"
echo "$OUTPUT_BASH"
echo "============================"
